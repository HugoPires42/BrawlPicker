...
type Props = {
  slug: string;
  /** Show the "← back to wiki" link at the top. Set false when embedded in a modal. */
  showBackLink?: boolean;
  /** Make brawler names in matchup lists clickable (modal-aware). */
  onPickBrawler?: (cubeName: string) => void;
};
...
export default function BrawlerDetail({
  ...
  if (!data || !data.brawler) {
    return (
      <div className="card text-bad text-sm">Brawler introuvable.</div>
    );
  }
  ...
  return (
    <section className="card">
      ...
      <ul className="space-y-1.5">
            ...
            <li key={m.name} className="flex items-center gap-2">
              ...
              {onPick ? (
                <button
                  ...
                  className="text-xs font-medium flex-1 truncate text-left hover:text-accent transition"
                >
                  {display}
                </button>
              ) : (
                <Link
                  href={`/wiki/brawlers/${m.name.toLowerCase().replace(/\s+/g, "-")}`}
        ...
        {items.length === 0 && <li className="text-[11px] text-muted">—</li>}
      </ul>
    </section>
  );
}
